...
try{
	...
	if (Monetbil::STATUS_SUCCESS == $payment_status) {
		//echo "Payement reussie";
		// Mark the order as paid in your system
		// Insertion du message à l'aide d'une requête préparée
		$payement="Payement reussie";
		$req = $bdd->prepare('INSERT INTO verification (service,transaction_id,phone,amount,status,country_name,country_code,operator,user,first_name,last_name,email,payement)
		VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)');
		$req->execute(array( $service,$transaction_id,$phone,$amount,$status,$country_name,$country_code,$operator,$user,$first_name,$last_name,$email,$payement));
	} elseif (Monetbil::STATUS_CANCELLED == $payment_status) {
		$payement="Payement annuler";
		$req = $bdd->prepare('INSERT INTO verification (service,transaction_id,phone,amount,status,country_name,country_code,operator,user,first_name,last_name,email,payement)
		VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)');
		$req->execute(array( $service,$transaction_id,$phone,$amount,$status,$country_name,$country_code,$operator,$user,$first_name,$last_name,$email,$payement));
	} else {
		$payement="Payement echouer";
		$req = $bdd->prepare('INSERT INTO verification (service,transaction_id,phone,amount,status,country_name,country_code,operator,user,first_name,last_name,email,payement)
		VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)');
		$req->execute(array( $service,$transaction_id,$phone,$amount,$status,$country_name,$country_code,$operator,$user,$first_name,$last_name,$email,$payement));
	}
